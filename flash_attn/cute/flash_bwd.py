...
from flash_attn.cute.tile_scheduler import ParamsBase, SingleTileScheduler, SingleTileVarlenScheduler, TileSchedulerArguments, TileSchedulerArgumentsBwd
...
class FlashAttentionBackwardSm80:
    ...
    def __call__(
        ...
        # Add fake batch dim = 1 mode to all varlen tensors
        mQ = cute.prepend_ones(mQ, 4) 
        mK = cute.prepend_ones(mK, 4) 
        mV = cute.prepend_ones(mV, 4) 
        ...
        mdK = cute.prepend_ones(mdK, 4) 
        mdV = cute.prepend_ones(mdV, 4) 
        ...
        if cutlass.const_expr(mCuSeqlensK is not None):
            ...
            tile_sched_args = TileSchedulerArgumentsBwd(
                num_block=0, # don't think this is used?
                num_head=cute.size(mQ.shape[2]),
                num_batch=cute.size(mQ.shape[0]), # check???
                headdim=cute.size(mQ.shape[3]),
                headdim_v=cute.size(mV.shape[3]),
                total_k=cute.size(),
            ...
            tile_sched_args = TileSchedulerArguments(
                num_block=cute.ceil_div(cute.size(mK.shape[1]), self.n_block_size),
                num_head=cute.size(mQ.shape[2]),
                num_batch=cute.size(mQ.shape[0]),
                seqlen_k = 0, # don't think this is used
                headdim = 0, # don't think this is used
                headdim_v = 0, # don't think this is used
                total_q = 0, # don't think this is used
                tile_shape_mn=(self.m_block_size, self.n_block_size), # don't think this is used
            )
    ...
    def kernel(
        ...
        if work_tile.is_valid_tile:
            ...
            # Use cutlass.cute.group_modes(input, begin: int, end: int = -1, *, loc=None, ip=None)
            if cutlass.const_expr(mCuSeqlensQ is not None):
                batch_offset_q = mCuSeqlensQ[batch_idx + 1] - mCuSeqlensQ[batch_idx]
            else:
                batch_offset_q = batch_idx * mQ.shape[1]

            if cutlass.const_expr(mCuSeqlensK is not None):
                batch_offset_k = mCuSeqlensK[batch_idx + 1] - mCuSeqlensK[batch_idx]
            else:
                batch_offset_k = batch_idx * mK.shape[1]



            # grouped_mQ = cute.group_modes(mQ, 0, 2)
    ...
    def load_Q_LSE(
        ...
        for m in cutlass.range_constexpr(cute.size(tQsQ.shape[1])):
            ...
            if self.is_even_m_smem_q or m < cute.size(tQsQ.shape[1]) - 1 or tQcQ[0, m, 0][0] < self.m_block_size:
                ...
                # TODO: Update predication here --> used for copying Q, also maybe if statement above
                # Actually... maybe I don't even need to do this?
