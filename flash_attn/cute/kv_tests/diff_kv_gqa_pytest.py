...
def flash_diff_kv_bwd_chunk(
    ...
    total_q_rounded_padded = (total_q + cu_seqlens.shape[0] * m_block_size - 1) // m_block_size * m_block_size
...
def _cuda_hygiene():
    ...
    if torch.cuda.is_available():
        ...
        del_vars = [v for v in list(globals()) if v.startswith('_tmp_')]
        for v in del_vars: globals().pop(v, None)
        gc.collect()
...
@pytest.mark.parametrize("n_heads", [1, 7])
@pytest.mark.parametrize("d_head", [64, 128])
@pytest.mark.parametrize("seq_len", [1, 32, 2048, 5000, 8192, 100000])
# @pytest.mark.parametrize("seq_len", [1, 32, 5000])
@pytest.mark.parametrize("n_chunks", [1, 7, 24])
# @pytest.mark.parametrize("page_size", [128, 256])
@pytest.mark.parametrize("page_size", [128, 256, 384])
@pytest.mark.parametrize("mha_type", ["mha", "mqa", "gqa"])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
@pytest.mark.parametrize("causal", [True])
def test_diff_kv_full(
    batch_size: int,
    n_heads: int,
    d_head: int,
    seq_len: int,
