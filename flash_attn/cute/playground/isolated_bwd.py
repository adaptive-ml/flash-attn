...
from flash_attn.cute import _flash_attn_bwd, FlashAttentionBackwardPreprocess, FlashAttentionBackwardSm80
...
def preprocess_caller():
    ...
    out = torch.randn(batch_size, seqlen_q, num_heads, head_dim_v,  device=device, dtype=dtype, requires_grad=True)
    dout = torch.randn(batch_size, seqlen_q, num_heads, head_dim_v,  device=device, dtype=dtype, requires_grad=True)

    lse = torch.randn(batch_size, num_heads, seqlen_q,  device=device, dtype=torch.float32, requires_grad=True)

    dq_accum = torch.empty(batch_size, num_heads, head_dim_rounded * seqlen_q_rounded,  device=device, dtype=torch.float32, requires_grad=True)
    dpsum = torch.empty(batch_size, num_heads, seqlen_q_rounded, dtype=torch.float32, device=device)
    lse_log2 = torch.empty(batch_size, num_heads, seqlen_q_rounded,  device=device, dtype=torch.float32, requires_grad=True)

    # 2. Relevant Asserts from interface.py
    assert out.shape == (batch_size, seqlen_q, num_heads, head_dim_v)
    assert dout.shape == (batch_size, seqlen_q, num_heads, head_dim_v)
    assert lse.shape == (batch_size, num_heads, seqlen_q), "lse must have shape (batch_size, num_head, seqlen_q)"
    assert out.dtype == dout.dtype, "inputs must have the same dtype"
    assert lse.dtype == torch.float32, "lse must be float32"
    assert all(t.is_cuda for t in (out, dout, lse)), "inputs must be on CUDA device"
    assert head_dim <= 256, "head_dim must be less than or equal to 256"
    alignment = 16 // out.element_size()
    ...
    dq_accum_tensor = dq_accum_tensor.mark_compact_shape_dynamic(mode=2, divisibility=8)
    ...
    dpsum_ref = dpsum_ref.permute([0, 2, 1])
    ...
    _stats('lse_log2', lse_log2, lse_log2_ref)
    _stats('dpsum', dpsum, dpsum_ref)
