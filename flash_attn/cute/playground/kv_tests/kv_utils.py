...
@torch.no_grad()
def reconstruct_paged_from_packed(
...
def generate_args(
    n_heads: int = 16,
    d_head: int = 128,
    seq_len: int = 128 * 32, # exact now since we have batch size 1
...
def generate_batched_args(
    ...
    seqused_q = torch.randint(min_seq_len, max_seq_len + 1, (batch_size,), dtype=torch.int32)
    seqused_k = torch.randint(min_seq_len, max_seq_len + 1, (batch_size,), dtype=torch.int32)
